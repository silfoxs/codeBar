...
状态栏弹层使用透明无边框 NSPanel，macOS 26+ 的弹层和设置页均使用不带 tint 的 `glassEffect(.regular)`，通过系统材质降低背景透出程度，提高文字和图表可读性。macOS 13–25 回退到系统 material，开启“减少透明度”时使用不透明背景。按钮保持无边框样式。点击外部、按 Escape 或失去焦点时关闭弹层，并释放事件监听。窗口根据菜单栏按钮所在屏幕定位并保留边缘间距。
