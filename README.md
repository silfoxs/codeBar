...
每个 provider 区域还会展示最近 7 天的日消耗柱状图，同时显示最近 30 天的官方合计值。当天日桶存在时，今日使用官方数据；当天日桶缺失时，应用会读取本机 `~/.codex/sessions` 和 `archived_sessions` 中的 `token_usage_record`，按 `response_id` 去重后显示“今日 · 本地估算”。本地估算只用于今日展示，不会加到官方 lifetime 或 30 天合计中；官方日桶出现后自动优先使用官方值。没有可用本地记录时，今日显示暂无数据。
